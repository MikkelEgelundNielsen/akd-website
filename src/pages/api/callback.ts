...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const mailgunFrom = env.MAILGUN_FROM || import.meta.env.MAILGUN_FROM
      || `AKD Danmark <noreply@${mailgunDomain}>`;
    ...
    formData.append('html', `
      ...
      <table style="border-collapse:collapse;font-family:sans-serif;font-size:15px;">
        <tr><td style="padding:6px 12px 6px 0;font-weight:bold;">Emne:</td><td>${esc(reason || 'Generel henvendelse')}</td></tr>
        <tr><td style="padding:6px 12px 6px 0;font-weight:bold;">Navn:</td><td>${esc(name)}</td></tr>
        <tr><td style="padding:6px 12px 6px 0;font-weight:bold;">Telefon:</td><td><a href="tel:${esc(phone)}">${esc(phone)}</a></td></tr>
    ...
    if (!mgRes.ok) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message: 'Der opstod en fejl ved afsendelse. Prøv venligst igen.',
          debug: { status: mgRes.status, body: errBody, url: mgUrl, from: mailgunFrom, to: toEmail },
        }),
